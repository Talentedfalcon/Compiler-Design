...
void fill_parsing_table(struct parsing_table* p){
    ...
    if(p->type==LL1){
        for(int i=0;i<p->num_non_term;i++){
            char** table_row=(char**)malloc((p->num_term)*sizeof(char*));
            int num_table_col=0;
            for(int j=0;j<p->num_term;j++){
                char* table_cell=(char*)malloc(1*sizeof(char));
                table_cell[0]=-1;
                table_row[num_table_col++]=table_cell;
            }
            p->table[num_table_row++]=table_row;
        }
    }
    else if(p->type==LR1){
        ...
        for(int i=0;i<p.num_non_term;i++){
            ...
            for(int j=0;j<p.num_term;j++){
                if(p.table[i][j][0]==-1){
                    ...
                    printf("%s\t",G.production_rules[p.table[i][j][0]-1][1]);
                }
            }
            printf("\n");
        }
